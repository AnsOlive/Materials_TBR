...
# Define enrichment range for simulation
...
# Main Simulation
##################################################################################################
# Loops through all the enrichments for a material
...
ax.plot(enrichments, TBR_values, linestyle='--')
ax.set_xlabel("Lithium Enrichment (%)")
ax.set_ylabel("TBR")
ax.set_title(f"TBR vs Enrichment for {salt_name}")
plt.grid(True)
...
with open(f'raw_data_{salt_name}.txt', 'w') as file:
    ...
    file.write(f"\n\n The maximum TBR for {salt_name} is {np.max(TBR_values)}")
    file.write(f"\n\n The mean TBR for {salt_name} is {np.mean(TBR_values)}")
    file.write(f"\n\n The median TBR for {salt_name} is {np.median(TBR_values)}")
    file.write("\n\nMaximum TBR Results:\n")
